...
public abstract class FrameworkServlet extends HttpServletBean implements ApplicationContextAware {
     ...
     *
     * 是否将 {@link #webApplicationContext} 设置到 {@link ServletContext} 的属性参数中
     */
	...
	protected WebApplicationContext initWebApplicationContext() {
		...
		// 第一种情况，如果 DispatcherServlet 构造方法已经传入 webApplicationContext 属性，则直接使用
	...
	protected void configureAndRefreshWebApplicationContext(ConfigurableWebApplicationContext wac) {
		...
		// 比较重要的一个监听器，监听应用上下文刷新事件，执行 FrameworkServlet 的 onApplicationEvent() 回调
		wac.addApplicationListener(new SourceFilteringListener(wac, new ContextRefreshListener()));

		// The wac environment's #initPropertySources will be called in any case when the context
		// is refreshed; do it eagerly here to ensure servlet property sources are in place for
		// use in any post-processing or initialization that occurs below prior to #refresh
		// 设置 ServletContext 和 ServletConfig 为环境属性
		ConfigurableEnvironment env = wac.getEnvironment();
		if (env instanceof ConfigurableWebEnvironment) {
			((ConfigurableWebEnvironment) env).initPropertySources(getServletContext(), getServletConfig());
		}

		// 目前是个空方法，暂无任何实现。允许让子类修改应用上下文
	...
	public void destroy() {
		...
		// 只支持本地管理的应用上下文调用 close() 方法，注入方式(setter)的上下文不支持
	...
	protected void service(HttpServletRequest request, HttpServletResponse response)
        ...
        // 调用父类，处理其它请求（实际上是调用当前类重写的doxxx方法）
		} else {
	...
	protected final void processRequest(HttpServletRequest request, HttpServletResponse response)
		...
		// 获取之前的与线程绑定的 LocaleContext 和 RequestAttributes
		// 创建一个新的 LocaleContext ，通过之前的请求参数创建一个新的 ServletRequestAttributes
	...
	private void initContextHolders(HttpServletRequest request,
		...
		// 初始化，将 LocaleContext 和 RequestAttributes 与当前线程绑定
