...
class AnnotationDrivenBeanDefinitionParser implements BeanDefinitionParser {
	...
	public BeanDefinition parse(Element element, ParserContext context) {
		...
		// 注册默认的 ContentNegotiationManager
		...
		if (element.hasAttribute("enable-matrix-variables")) {
			...
			// 指定 RequestMappingHandlerMapping 不移除分号内容
			handlerMappingDef.getPropertyValues().add("removeSemicolonContent", !enableMatrixVariables);
		}

		// 为 RequestMappingHandlerMapping 配置路径匹配等属性
		configurePathMatchingProperties(handlerMappingDef, element, context);
		readerContext.getRegistry().registerBeanDefinition(HANDLER_MAPPING_BEAN_NAME , handlerMappingDef);

		// 注册默认的 CorsConfiguration 的 LinkedHashMap
		RuntimeBeanReference corsRef = MvcNamespaceUtils.registerCorsConfigurations(null, context, source);
		handlerMappingDef.getPropertyValues().add("corsConfigurations", corsRef);

		// 选择性注册 ConversionService / Validator / MessageCodesResolver
		RuntimeBeanReference conversionService = getConversionService(element, source, context);
		RuntimeBeanReference validator = getValidator(element, source, context);
		RuntimeBeanReference messageCodesResolver = getMessageCodesResolver(element);

		// 一个配置类，用于初始化一些默认的 WebDataBinder 属性
		...
		// 注册 HttpMessageConverter / HandlerMethodArgumentResolver / HandlerMethodReturnValueHandler
		ManagedList<?> messageConverters = getMessageConverters(element, source, context);
		ManagedList<?> argumentResolvers = getArgumentResolvers(element, context);
		ManagedList<?> returnValueHandlers = getReturnValueHandlers(element, context);

		// 解析 mvc:async-support
		...
		// 为 RequestMappingHandlerAdapter 添加配置属性
	...
	private RuntimeBeanReference getConversionService(Element element, @Nullable Object source, ParserContext context) {
		...
		if (element.hasAttribute("conversion-service")) {
			...
			// 构造一个默认的 FormattingConversionServiceFactoryBean
		...
		// 返回 ConversionServiceXXX bean引用
	...
	private RuntimeBeanReference getValidator(Element element, @Nullable Object source, ParserContext context) {
		...
		// 注册默认的 OptionalValidatorFactoryBean
	...
	private void configurePathMatchingProperties(
		...
		if (pathMatchingElement != null) {
			...
			// 设置 RequestMappingHandlerMapping 的三个属性
			...
			// 设置 RequestMappingHandlerMapping 的 UrlPathHelper 和 AntPathMatcher
	...
	private RuntimeBeanReference getMessageCodesResolver(Element element) {
		// 用于将错误代码转换为消息，通过 DataBinder（默认有4种错误消息）
	...
	private ManagedList<Object> wrapLegacyResolvers(List<Object> list, ParserContext context) {
		...
		for (Object object : list) {
			// <bean /> 用 ServletWebArgumentResolverAdapter 适配一下
