...
public abstract class AnnotatedElementUtils {
	...
	public static AnnotationAttributes getMergedAnnotationAttributes(
		...
		// 获取 element 上的 annotationType 注解类型的 所有属性值
	...
	public static <A extends Annotation> A findMergedAnnotation(AnnotatedElement element, Class<A> annotationType) {
		...
		A annotation = element.getDeclaredAnnotation(annotationType);
		if (annotation != null) {
			// 返回实现该 annotation 接口的JDK动态代理对象（注解也可以被实现）
			return AnnotationUtils.synthesizeAnnotation(annotation, element);
		}
	...
	private static <T> T searchWithGetSemanticsInAnnotations(@Nullable AnnotatedElement element,
		...
		for (Annotation annotation : annotations) {
			...
			// 判断当前注解是否 不在 java.lang.annotation 包中
		...
		for (Annotation annotation : annotations) {
			Class<? extends Annotation> currentAnnotationType = annotation.annotationType();
			// 判断注解是否 不在 java / org.springframework.lang 包中
