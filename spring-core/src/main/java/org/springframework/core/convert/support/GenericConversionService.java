...
public class GenericConversionService implements ConfigurableConversionService {
	...
	public void addConverter(Converter<?, ?> converter) {
	    // 获取 ResolvableType 对象，从 converter 上获取
		ResolvableType[] typeInfo = getRequiredTypeInfo(converter.getClass(), Converter.class);
		// 如果获取不到，并且 converter 是 DecoratingProxy 类型，则基于 ((DecoratingProxy) converter).getDecoratedClass() 类
		...
		// 封装成 ConverterAdapter 对象，添加到 converters 中
	...
	public void addConverterFactory(ConverterFactory<?, ?> factory) {
		// 和上方的 addConverter(Converter<?, ?>) 一样的逻辑
	...
	/**
	 * 是否可以跳过该类型转换 -> sourceType.isAssignableTo(targetType)
	 *
	...
	protected GenericConverter getConverter(TypeDescriptor sourceType, TypeDescriptor targetType) {
		...
		// 如果查找不到，则获得默认的 Converter 对象  NO_OP
		if (converter == null) {
	...
	/**
	 * 清空 getConverter() 方法获取到的 Converter 缓存
	 */
	...
	private final class ConverterAdapter implements ConditionalGenericConverter {
		...
		/**
		 * 转换类型对，见构造，该类为 GenericConverter 接口的内部类
		 */
	...
	private static class Converters {
		...
		public void add(GenericConverter converter) {
		    // 获得 ConvertiblePair 集合，用来获取 ConvertersForPair 对象
			...
			if (convertibleTypes == null) {
			    ...
			    // 通过迭代的方式 依次(LinkedList) 添加到【converters】中
				for (ConvertiblePair convertiblePair : convertibleTypes) {
				    // 从 converters 中，获得 ConvertersForPair 对象，需要通过 ConvertiblePair 为key来获取
					// 该对象是用来管理 相同转换行为的 Converter 对象
		...
		/**
		 * 获取 类型对 对应的 类型对管理者（存有相同 源类型->目标类型）
		 */
	...
	private static class ConvertersForPair {
		...
		public GenericConverter getConverter(TypeDescriptor sourceType, TypeDescriptor targetType) {
		    ...
		    for (GenericConverter converter : this.converters) {
				if (!(converter instanceof ConditionalGenericConverter) || // 非条件 ConditionalGenericConverter
						((ConditionalGenericConverter) converter).matches(sourceType, targetType)) { // 是条件，则匹配
					return converter;
				}
			}
			return null;
