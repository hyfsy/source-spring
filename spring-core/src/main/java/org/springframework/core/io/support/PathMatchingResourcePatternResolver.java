...
public class PathMatchingResourcePatternResolver implements ResourcePatternResolver {
	...
	protected Resource[] findAllClassPathResources(String location) throws IOException {
		...
		// 真正执行加载所有 classpath 资源 -> UrlResource
	...
	protected Set<Resource> doFindPathMatchingJarResources(Resource rootDirResource, URL rootDirURL, String subPattern)
		...
		// 如果是 JarURLConnection ，则直接获取 jar 包信息
        ...
        // 如果不是 JarURLConnection，就解析路径，解析相关需要的 jar 包信息
		} else {
		...
		try {
			...
			// 读取所有 jar 里面的文件，然后与路径进行配置，如果配置成功就添加到结果中进行返回
	...
	protected Set<Resource> doFindPathMatchingFileResources(Resource rootDirResource, String subPattern)
		...
		File rootDir;
		try {
			// 将 Resource 对象转换为 File 对象，获取绝对路径对应的 file
		...
		// 如果存在匹配的文件，会返回一个 FileSystemResource 集合
