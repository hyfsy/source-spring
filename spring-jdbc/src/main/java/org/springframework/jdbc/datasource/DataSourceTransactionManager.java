...
public class DataSourceTransactionManager extends AbstractPlatformTransactionManager
    ...
    /**
     * 强制只读。默认为 false
	 * 对于一些特殊的数据库，只读设置在手动提交之前的，只读状态可能会失效。强制提交会在手动提交设置后才设置
     */
	...
	protected Object doGetTransaction() {
		...
		// 设置当前是否是否支持保存点（支持嵌套事务也就意味着支持保存点）
		txObject.setSavepointAllowed(isNestedTransactionAllowed());
		// 如果当前线程，已经记录数据库连接，则使用原有连接
        // 【重要】，这就是为什么，在事务中，多数据源无法切换
        // 至于什么时候，绑定到当前线程，可以看 #doBegin() 方法，开始事务
		ConnectionHolder conHolder =
				(ConnectionHolder) TransactionSynchronizationManager.getResource(obtainDataSource());
		// 到现在还没创建新的 ConnectionHolder 所以，手动设置此状态为 false
		// 该对象往往和 Connection 对象一起创建
	...
	protected void doBegin(Object transaction, TransactionDefinition definition) {
		...
		try {
			// 没有连接 或者 连接对象的事务已经与当前线程绑定，则需要获取新的数据库连接
			if (!txObject.hasConnectionHolder() ||
					txObject.getConnectionHolder().isSynchronizedWithTransaction()) {
				// 获得数据库连接，懒加载的数据库也会在此初始化（比如druid）
			...
			// 设置 synchronizedWithTransaction 属性，表示该连接与线程同步了
			txObject.getConnectionHolder().setSynchronizedWithTransaction(true);
			// 获得数据库连接
			con = txObject.getConnectionHolder().getConnection();

			// 【1. 设置 connection 的属性】设置 只读状态 和 隔离级别
	...
	protected void doCleanupAfterCompletion(Object transaction) {
        ...
        // 从 TransactionSynchronizationManager 中，移除当前线程绑定的 ConnectionHolder
		if (txObject.isNewConnectionHolder()) {
	...
	protected void prepareTransactionalConnection(Connection con, TransactionDefinition definition)
		...
		// 一些数据库的特殊性，在 autoCommit 前设置的只读属性可能会失效
		// 所以此处判断是否通过数据库语句来 强制只读事务
