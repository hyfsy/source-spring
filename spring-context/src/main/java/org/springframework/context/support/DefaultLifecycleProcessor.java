...
public class DefaultLifecycleProcessor implements LifecycleProcessor, BeanFactoryAware {
	...
	private void startBeans(boolean autoStartupOnly) {
		...
		lifecycleBeans.forEach((beanName, bean) -> {
			// 默认只有 SmartLifecycle 类型的bean才会分组（忽略Lifecycle类型）
	...
	private void stopBeans() {
		Map<String, Lifecycle> lifecycleBeans = getLifecycleBeans();
		Map<Integer, LifecycleGroup> phases = new HashMap<>();
		// 对所有生命周期对象 通过phase分组 停止
		lifecycleBeans.forEach((beanName, bean) -> {

			// 和 startBeans(boolean autoStartupOnly) 不同，此处会为所有生命周期bean分组
	...
	private void doStop(Map<String, ? extends Lifecycle> lifecycleBeans, final String beanName,
		...
		Lifecycle bean = lifecycleBeans.remove(beanName);
		if (bean != null) {
			// 当前bean依赖的bean先执行，和 doStart() 方法相反
			String[] dependentBeans = getBeanFactory().getDependentBeans(beanName);
			for (String dependentBean : dependentBeans) {
				doStop(lifecycleBeans, dependentBean, latch, countDownBeanNames);
			}
			try {
				// start() 和 stop() 在此方法的限制处只能执行一种
				// 但是控制 isRunning() 的返回值就可以两种都执行
				// Lifecycle 在容器刷新时不会执行 start()，需要注意
				if (bean.isRunning()) {
					if (bean instanceof SmartLifecycle) {
						...
						// 内部调用 Lifecycle 的 stop() 方法，并递减线程计数器
