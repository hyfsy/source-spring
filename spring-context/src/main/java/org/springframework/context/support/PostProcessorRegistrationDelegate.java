...
final class PostProcessorRegistrationDelegate {
	...
	public static void invokeBeanFactoryPostProcessors(ConfigurableListableBeanFactory beanFactory, List<BeanFactoryPostProcessor> beanFactoryPostProcessors) {
        ...
        // 已经执行过处理方法的 BeanDefinitionRegistryPostProcessor 的集合
		...
		if (beanFactory instanceof BeanDefinitionRegistry) {
			...
			// 一、执行 BeanDefinitionRegistryPostProcessor 的处理方法
			// 执行三次，分别执行事先添加到 AbstractApplicationContext 的、单例池中存在的（两种排序方式）、后续可能有添加的
            ...
            // 第一步，获得单例池中添加的，且实现 PriorityOrdered 接口的 BeanDefinitionRegistryPostProcessor 数组
			...
			// 一、执行所有单例池中的实现了 PriorityOrdered 接口的BeanDefinitionRegistryPostProcessor的处理方法
            ...
            // 添加到 registryProcessors 中
            registryProcessors.addAll(currentRegistryProcessors);
			// 一、执行所有单例池中的实现了 Ordered 接口的BeanDefinitionRegistryPostProcessor的处理方法
            invokeBeanDefinitionRegistryPostProcessors(currentRegistryProcessors, registry);
            // 清空 currentRegistryProcessors
            currentRegistryProcessors.clear();
			...
			while (reiterate) {
                ...
                // 添加到 registryProcessors 中
                registryProcessors.addAll(currentRegistryProcessors);
				// 一、执行可能还有的实现了 Ordered 接口的 BeanDefinitionRegistryPostProcessor 的处理方法
                invokeBeanDefinitionRegistryPostProcessors(currentRegistryProcessors, registry);
                // 清空 currentRegistryProcessors
                currentRegistryProcessors.clear();
			}

			// 二、执行 BeanFactoryPostProcessor 和 BeanDefinitionRegistryPostProcessor 的共同的处理方法
		...
		// Do not initialize FactoryBeans here: We need to leave all regular beans
		// uninitialized to let the bean factory post-processors apply to them!
		// 注册剩下的 BeanFactoryPostProcessor 类型
	...
	public static void registerBeanPostProcessors(
		...
		sortPostProcessors(priorityOrderedPostProcessors, beanFactory);
        // 后注册
		// 一、首先注册所有的 BeanPostProcessor ，其次注册所有的 MergedBeanDefinitionPostProcessor
	 ...
	 * @see 初始注册bean：{@link org.springframework.context.annotation.ConfigurationClassPostProcessor}
	 */
	 ...
	 * 在 BeanPostProcessor 创建过程中拦截其他被创建的 bean对象，打印日志
	 * 主要为了防止一个bean的创建没有经过所有的后处理器
	 */
