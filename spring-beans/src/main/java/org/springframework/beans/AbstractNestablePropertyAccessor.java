...
public abstract class AbstractNestablePropertyAccessor extends AbstractPropertyAccessor {
	...
	/**
	 * 主要逻辑：
	 * 1. 通过 propertyName 获取到 AbstractNestablePropertyAccessor 对象，一般来说就是一个 BeanWrapperImpl
	 * 2. 通过最终的属性路径创建出 PropertyTokenHolder 对象，包含属性名称和可能存在的key（如 map）
	 * 3. 通过 token 设置属性值
	 */
	@Override
	...
	/**
	 * 和上面类似，最终都调用了 setPropertyValue(tokens, PropertyValue) 方法设置属性值
	 */
	@Override
	public void setPropertyValue(PropertyValue pv) throws BeansException {
		...
		if (tokens == null) {
			...
			AbstractNestablePropertyAccessor nestedPa;
			try {
				// 通过属性路径 递归获取最内部的属性访问者（this / BeanWrapperImpl）
				nestedPa = getPropertyAccessorForPropertyPath(propertyName);
			}
			catch (NotReadablePropertyException ex) {
				throw new NotWritablePropertyException(getRootClass(), this.nestedPath + propertyName,
			...
			// 嵌套属性的 tokens并不会被缓存起来，每次通过属性路径设置值时，都会创建一个新的 tokens
			tokens = getPropertyNameTokens(getFinalPath(nestedPa, propertyName));
			if (nestedPa == this) {
				// 属性没有嵌套属性时，才将tokens缓存在 PropertyValue 中
				pv.getOriginalPropertyValue().resolvedTokens = tokens;
			}
			// 通过 属性路径 和 属性值 设置进当前对象包装的 bean 中
	...
	private void processKeyedProperty(PropertyTokenHolder tokens, PropertyValue pv) {
		...
		if (propValue.getClass().isArray()) {
			...
			Object convertedValue = convertIfNecessary(tokens.canonicalName, oldValue, pv.getValue(),
					requiredType, ph.nested(tokens.keys.length));
			int size = list.size();
			// 比如此处只有一个 1 索引位置的值，但集合大小不够大，则前面的值将自动填充 null
		...
		try {
			...
			if (!Boolean.FALSE.equals(pv.conversionNecessary)) {
				...
				if (pv.isConverted()) {
					...
					// 没转换过，再转换一下
					// oldValue 也就用作 editor.setValue(oldValue) 使用
					valueToApply = convertForProperty(
							tokens.canonicalName, oldValue, originalValue, ph.toTypeDescriptor());
				}
				// 一般设置 conversionNecessary 为 true
	...
	protected Object getPropertyValue(PropertyTokenHolder tokens) throws BeansException {
		...
		// 通过 BeanWrapper 获取新创建的 PropertyHandler ，内部包含属性描述器、属性类型、可读可写
		PropertyHandler ph = getLocalPropertyHandler(actualName);
		if (ph == null || !ph.isReadable()) {
			throw new NotReadablePropertyException(getRootClass(), this.nestedPath + propertyName);
		}
		try {
			// 通过内部的属性描述器获取 readMethod ，调用 getter获取属性值
	...
	protected AbstractNestablePropertyAccessor getPropertyAccessorForPropertyPath(String propertyPath) {
		// 获取属性名称分隔符的索引
		// propertyPath 为xml中的 property -> name 属性
	...
	private AbstractNestablePropertyAccessor getNestedPropertyAccessor(String nestedProperty) {
		...
		// 通过 nestedProperty 构造一个 PropertyTokenHolder 新的对象，内部会处理集合类型属性 [..]
		PropertyTokenHolder tokens = getPropertyNameTokens(nestedProperty);
		String canonicalName = tokens.canonicalName;
		// 调用 getter 获取属性值
		Object value = getPropertyValue(tokens);
		if (value == null || (value instanceof Optional && !((Optional) value).isPresent())) {
			// 是否为不存在的路径设置默认值，默认为 false
			if (isAutoGrowNestedPaths()) {
		...
		if (nestedPa == null || nestedPa.getWrappedInstance() != ObjectUtils.unwrapOptional(value)) {
			...
			// 由子类扩展 创建一个新的内部属性访问者（BeanWrapperImpl）
	...
	private PropertyTokenHolder getPropertyNameTokens(String propertyName) {
		...
		// propertyName = actualName[key]...[key]
		...
		// 通过 actualName 构造一个 PropertyTokenHolder 实例
		// canonicalName 默认值和 actualName 一样
		PropertyTokenHolder tokens = new PropertyTokenHolder(actualName != null ? actualName : propertyName);
		if (!keys.isEmpty()) {
			// canonicalName[key][key][key]
			tokens.canonicalName += PROPERTY_KEY_PREFIX +
					StringUtils.collectionToDelimitedString(keys, PROPERTY_KEY_SUFFIX + PROPERTY_KEY_PREFIX) +
					PROPERTY_KEY_SUFFIX;
			tokens.keys = StringUtils.toStringArray(keys);
		}
		// 返回新创建的 PropertyTokenHolder
		return tokens;
	}
