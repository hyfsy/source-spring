...
public class PropertyEditorRegistrySupport implements PropertyEditorRegistry {
	...
	/**
	 * 类型与 父/接口类型映射的属性编辑器 的映射
	 * <p>
	 * @see #getCustomEditor(Class)
	 */
	@Nullable
	...
	public PropertyEditor getDefaultEditor(Class<?> requiredType) {
		...
		if (this.defaultEditors == null) {
			// 创建一些默认的属性编辑器
			// 如果还开启了 configValueEditorsActive ，则还会注册数组类型的默认属性编辑器
			// 一般不需要开启，因为该属性编辑器并不适用于数据绑定
	...
	public PropertyEditor findCustomEditor(@Nullable Class<?> requiredType, @Nullable String propertyPath) {
		...
		if (propertyPath != null) {
			// 1.先尝试通过 属性路径和需要的类型 从 customEditorsForPath 中查找
			if (this.customEditorsForPath != null) {
			...
			// 2.没有必要类型，只通过属性路径获取必要类型 - 抽象方法，供子类实现
			...
			this.customEditorsForPath.forEach((editorPath, editorHolder) -> {
				if (nestedProperty != null) {
					...
					if (pos != -1) {
						...
						// 通过第一个 [.] 获取第一个属性的名称
						// 和传入的 nestedProperty 比较，相同则注册该对应的属性路径编辑器
	...
	/**
	 * map[key][v][v2] ->
	 * [map[v][v2], map[v2], map, map[v], map[key][v2], map[key], map[key][v]]
	 * <p>
	...
	private static final class CustomEditorHolder {
		...
		private PropertyEditor getPropertyEditor(@Nullable Class<?> requiredType) {
			...
			// for elements.)
			// registeredType 不为null , 该类型和 requiredType 为实现或继承关系
			// registeredType 为 null , 该类型不为 集合 或 数组 类型
