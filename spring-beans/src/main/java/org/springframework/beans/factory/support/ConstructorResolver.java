...
class ConstructorResolver {
	...
	public BeanWrapper autowireConstructor(String beanName, RootBeanDefinition mbd,
		...
		if (explicitArgs != null) {
			...
			if (argsToResolve != null) {
				// 处理依赖bean，比如 @Autowired/@Qualifier等等
		...
		if (constructorToUse == null || argsToUse == null) {
            ...
            // 如果候选的 chosenCtors 未传入，则获取构造方法们
			...
			for (Constructor<?> candidate : candidates) {
                ...
                // 因为，已经按照参数个数降序排列了，找到了匹配的构造函数，并且参数也是最多的
				...
				if (resolvedValues != null) {
					try {
                        // 获取 @ConstructorProperties 注解上的参数名称
						String[] paramNames = ConstructorPropertiesChecker.evaluate(candidate, paramTypes.length);
						if (paramNames == null) {
                            // 获取构造函数、方法参数的探测器
							ParameterNameDiscoverer pnd = this.beanFactory.getParameterNameDiscoverer();
							if (pnd != null) {
                                // 通过探测器获取构造函数的参数名称，底层通过class文件流的方式
                ...
                // 严格模式：解析构造函数时，存在相同权重的构造函数时，则抛出异常
				...
				if (typeDiffWeight < minTypeDiffWeight) {
					...
					// 存在相同权重的构造函数，则添加到 ambiguousConstructors 中
            ...
            // 没有可执行的工厂方法 或 存在相同权重，并且处于严格模式下，则抛出异常
		...
		if (factoryMethodToUse == null || argsToUse == null) {
			...
			if (explicitArgs != null) {
				...
				if (mbd.hasConstructorArgumentValues()) {
                    ...
                    // 解析构造函数的参数
                    // 将该 bean 的构造函数参数解析为 resolvedValues 对象，其中会涉及到其他 bean
					// 将 cargs 中的参数按顺序添加到 resolvedValues 中，返回参数个数
					minNrOfArgs = resolveConstructorArguments(beanName, mbd, bw, cargs, resolvedValues);
				} else {
					minNrOfArgs = 0;
				}
				...
				if (paramTypes.length >= minNrOfArgs) {
					...
					if (explicitArgs != null) {
                        ...
                        try {
                            ...
                            // ParameterNameDiscoverer 是用于解析方法和构造函数的参数名称的接口，为参数名称发现者
                            ParameterNameDiscoverer pnd = this.beanFactory.getParameterNameDiscoverer();
                            // 获取指定构造函数的参数名称
							if (pnd != null) {
								paramNames = pnd.getParameterNames(candidate);
	...
	private int resolveConstructorArguments(String beanName, RootBeanDefinition mbd, BeanWrapper bw,
		...
		// 此处的 BeanDefinitionValueResolver 用于转换参数类型
	...
	private ArgumentsHolder createArgumentArray(
		...
		for (int paramIndex = 0; paramIndex < paramTypes.length; paramIndex++) {
			...
			if (valueHolder != null) {
				...
				// 包装 方法 或 构造函数
				// 注意此处的 paramIndex ，通过此属性获取到 当前方法签名上的对应的属性的索引，用于下方获取属性值
				MethodParameter methodParam = MethodParameter.forExecutable(executable, paramIndex);
