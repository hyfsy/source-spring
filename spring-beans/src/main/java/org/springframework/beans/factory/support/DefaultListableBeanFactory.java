...
public class DefaultListableBeanFactory extends AbstractAutowireCapableBeanFactory
	...
	// 两个不同bean名称 但却有相同的alias ,是否用后面新的覆盖前面定义好的
				...
				try {
					...
					if (!mbd.isAbstract() && (allowEagerInit ||
						...
						boolean matchFound =
								...
								isTypeMatch(beanName, type); // 核心的匹配类型逻辑
	...
	public void freezeConfiguration() {
		...
		// 设置 frozenBeanDefinitionNames 不可变
	...
	public void preInstantiateSingletons() throws BeansException {
        ...
        // Spring 4.1 新增的特性
		/*
		 * @see #{@link EventListenerMethodProcessor} 处理
		 */
		for (String beanName : beanNames) {
	...
	public void registerBeanDefinition(String beanName, BeanDefinition beanDefinition)
		...
		// 容器中已经存在之前bean定义创建出的单例bean，需要先销毁
        // 重新设置 beanName 对应的缓存
		if (existingDefinition != null || containsSingleton(beanName)) {
			// 销毁bean单例和合并bean定义，也会回调后处理，清空关于当前bean定义的所有缓存信息
	...
	public void removeBeanDefinition(String beanName) throws NoSuchBeanDefinitionException {
		...
		// 如果已经有bean在创建过程中，则这里需要同步处理
		if (hasBeanCreationStarted()) {
			// Cannot modify startup-time collection elements anymore (for stable iteration)
			synchronized (this.beanDefinitionMap) {
				// 感觉这里有点 CopyOnWrite 的思想
	...
	protected void resetBeanDefinition(String beanName) {
		...
		// 一、 销毁指定bean定义时的回调（主要用在单例bean已经存在但又注册了该bean定义时调用）
	...
	public Object resolveDependency(DependencyDescriptor descriptor, @Nullable String requestingBeanName,
        ...
        // javaxInjectProviderClass 类注入的特殊处理
            ...
            // 为实际依赖关系目标的延迟加载对象 构建代理
            // 默认实现返回 null
			Object result = getAutowireCandidateResolver().getLazyResolutionProxyIfNecessary(descriptor, requestingBeanName);
			if (result == null) {
                // 通用处理逻辑，返回解析后的依赖对象（依赖注入注解的处理）
	...
	public Object doResolveDependency(DependencyDescriptor descriptor, @Nullable String beanName,
		...
		// 处理 @Value | @Autowired | @Qualifier 返回对应的依赖对象
		...
		try {
            ...
            // 依赖的类型
			// 如果 descriptor 中包装的为属性，则直接获取类型，如果包装的为方法，则通过其内部的索引获取签名中对应的属性类型
			...
			// 处理 @Autowired 通过类型匹配到多个的情况
			if (matchingBeans.size() > 1) {
                // 确认给定 bean autowire 的候选者
                // 按照 @Primary 和 @PriorityOrder 的顺序
				// 再按照【名称】匹配
			...
			// 如果依赖的bean候选为Class类型，则创建该bean实例
	...
	protected Map<String, Object> findAutowireCandidates(
		...
		for (Map.Entry<Class<?>, Object> classObjectEntry : this.resolvableDependencies.entrySet()) {
			...
			if (autowiringType.isAssignableFrom(requiredType)) {
				...
				// 处理 autowiringValue 为 ObjectFactory 的情况（需要进行代理）
		...
		for (String candidate : candidateNames) {
			// 判断不是自己引用自己 & 自动装配的候选（包括 @Qualifier 的处理）
			if (!isSelfReference(beanName, candidate) && isAutowireCandidate(candidate, descriptor)) {
				// 通过候选的名称获取bean，并放入候选列表(result)，此处并不代表这 首先通过名称获取
	...
	protected String determineAutowireCandidate(Map<String, Object> candidates, DependencyDescriptor descriptor) {
		...
		// 获取 Primary 标识的bean名称
		String primaryCandidate = determinePrimaryCandidate(candidates, requiredType);
		if (primaryCandidate != null) {
			return primaryCandidate;
		}

		// 确定最高优先级的bean名称，没有则为null
