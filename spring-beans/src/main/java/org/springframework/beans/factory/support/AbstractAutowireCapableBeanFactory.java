...
public abstract class AbstractAutowireCapableBeanFactory extends AbstractBeanFactory
	...
	private boolean allowCircularReferences = true; // 是否允许 Bean 之间存在循环依赖
	...
	public AbstractAutowireCapableBeanFactory() {
		super();
		// 设置忽略指定的依赖
		// 详细见 DefaultSingletonBeanRegistry -> dependentBeanMap、dependenciesForBeanMap
	...
	protected Object createBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args)
        ...
        // 确保此时的 bean 已经被解析了
        // 主要是因为该动态解析(SpEL)的 class 无法保存到共享的 BeanDefinition
		...
		try {
            // 验证和准备覆盖方法（@Lookup/MethodReplacer）
	...
	protected Object doCreateBean(final String beanName, final RootBeanDefinition mbd, final @Nullable Object[] args)
        ...
        // 使用合适的实例化策略来创建新的实例：自定义策略、工厂方法、构造函数自动注入、简单初始化
		...
		synchronized (mbd.postProcessingLock) {
			if (!mbd.postProcessed) {
				try {
                    // 五、后置处理修改合并后的 BeanDefinition ，实例创建完毕，注入属性前
		...
		if (earlySingletonExposure) {
            ...
            // 这里是为了后期避免循环依赖
			// 六、必要时通过 ObjectFactory 获取早期bean引用（getEarlyBeanReference内部）
		...
		// 循环依赖处理
		if (earlySingletonExposure) {
            // 获取 earlySingletonReference，为依赖对象（不完整）的一个引用 A->B B->A | A->B->A(Get Early)
	...
	protected Object resolveBeforeInstantiation(String beanName, RootBeanDefinition mbd) {
		...
		if (!Boolean.FALSE.equals(mbd.beforeInstantiationResolved)) {
			...
			// 非用户定义的bean定义 并且 当前上下文中存在实例化回调后处理器
			if (!mbd.isSynthetic() && hasInstantiationAwareBeanPostProcessors()) {
				Class<?> targetType = determineTargetType(beanName, mbd);
				if (targetType != null) {
				    // 二、实例化前处理，此时bean定义（在容器中，可获取）已经准备就绪
					bean = applyBeanPostProcessorsBeforeInstantiation(targetType, beanName);
					if (bean != null) {
					    // 三、初始化后处理（aop相关，实例化前后可能返回了一个代理对象）
	...
	protected Object applyBeanPostProcessorsBeforeInstantiation(Class<?> beanClass, String beanName) {
		for (BeanPostProcessor bp : getBeanPostProcessors()) {
			if (bp instanceof InstantiationAwareBeanPostProcessor) {
				...
				// 执行实例化前处理，如果存在返回值，则直接返回 并用该返回值替换原本的bean实例
	...
	protected BeanWrapper createBeanInstance(String beanName, RootBeanDefinition mbd, @Nullable Object[] args) {
		...
		// 通过 自定义实例化策略 | 工厂方法 | 构造函数注入 | 默认构造实例化 获取 BeanWrapper 对象
        ...
        if (resolved) {
			...
			// 四、通过 SmartInstantiationAwareBeanPostProcessor 确定创建 bean使用的构造函数
		Constructor<?>[] ctors = determineConstructorsFromBeanPostProcessors(beanClass, beanName);
        // 有参数情况时，创建 Bean 。先利用参数个数，类型等，确定最精确匹配的构造方法。
		// 注：建议看过 AutowiredAnnotationBeanPostProcessor 的执行流程后再看此处
        ...
        // 类型为泛型，则获取泛型内类型的所有的构造方法，非泛型返回null
        ...
        // 没获取到候选的构造方法，则只能调用无参构造方法来创建实例了(兜底方法)
        return instantiateBean(beanName, mbd);
	}
	...
	protected void populateBean(String beanName, RootBeanDefinition mbd, @Nullable BeanWrapper bw) {
        ...
        // 在设置属性之前通过 InstantiationAwareBeanPostProcessors 给予最后一次改变 bean 的机会
		...
		// 属性的自动注入 - 半核心
		// 此处只处理 明确声明的 注入方式（byName | byType），默认为 NO
        ...
        // 是否需要进行【依赖检查】
		// dependencyCheck 在 RootBeanDefinition 实例化时传入构造函数中
		...
		if (hasInstAwareBpps) {
			...
			for (BeanPostProcessor bp : getBeanPostProcessors()) {
				if (bp instanceof InstantiationAwareBeanPostProcessor) {
					InstantiationAwareBeanPostProcessor ibp = (InstantiationAwareBeanPostProcessor) bp;
					// 在明确的 byName/byType 设置属性值之后
                    // 八、通过后处理 对bean的属性值做一些处理，并返回该bean的所有【属性值】，在依赖检查前
					...
					if (pvsToUse == null) {
						...
						// 九、postProcessProperties 没有属性值返回时，才调用此后置处理
						// 		通过该bean的所有属性描述器对可能的【属性值】进行进一步处理，在依赖检查前
						pvsToUse = ibp.postProcessPropertyValues(pvs, filteredPds, bw.getWrappedInstance(), beanName);
                        // 如果实在没有属性，直接返回，不进行依赖检查和设置属性的操作
						if (pvsToUse == null) {
							return;
						}
					}
        ...
        // 将属性应用到 bean 中
		if (pvs != null) {
			// 通过BeanWrapperImpl 的内部类 BeanPropertyHandler 调用 getter/setter 获取/设置属性值
		...
		for (String propertyName : propertyNames) {
			try {
				...
				if (Object.class != pd.getPropertyType()) {
                    // 探测指定属性的 set 方法
					// 按照类型注入，很显然，使用setter方法设置，通过 DependencyDescriptor 实现
	...
	protected PropertyDescriptor[] filterPropertyDescriptorsForDependencyCheck(BeanWrapper bw, boolean cache) {
		...
		if (filtered == null) {
			// 获取 BeanWrapper 的所有 PropertyDescriptor（有全局过滤规则）
	...
	protected void checkDependencies(
		...
		// 所有 PropertyDescriptor 进行依赖检查
		for (PropertyDescriptor pd : pds) {
			// 有 setter 方法 但是没有属性值，则需要抛异常
	...
	protected void applyPropertyValues(String beanName, BeanDefinition mbd, BeanWrapper bw, PropertyValues pvs) {
		...
		for (PropertyValue pv : original) {
            ...
            // 属性值需要转换
				...
				// 解析出实际的属性值（xml引用等），如果给定类型，则顺便转换为该类型
				...
				if (resolvedValue == originalValue) {
					if (convertible) {
						// 定义返回的值一样 并且 属性可以转换，说明不需要转换，值直接设置
	...
	private Object convertForProperty(
		...
		// 最后走 typeConverterDelegate.convertIfNecessary 进行真正的转换
	...
	protected Object initializeBean(final String beanName, final Object bean, @Nullable RootBeanDefinition mbd) {
		...
		if (mbd == null || !mbd.isSynthetic()) {
			// 十、初始化前处理，设置属性值完毕后调用，在调用自定义 init 方法前
			// 内部会调用 @PostConstruct 声明的生命周期方法(支持的话) & 处理六种 Aware 接口回调
		...
		// 该bean定义为应用程序创建的
		if (mbd == null || !mbd.isSynthetic()) {
			// 十一、初始化后处理，设置属性值完毕后调用，在调用自定义 init 方法后
	...
	protected void invokeInitMethods(String beanName, final Object bean, @Nullable RootBeanDefinition mbd)
		...
		// 1.调用 InitializingBean 回调方法
		// 2.调用自定义 init-method 方法
	...
	protected void invokeCustomInitMethod(String beanName, final Object bean, RootBeanDefinition mbd)
		...
		// 支持访问私有方法时才会获取所有的方法
		// 否则仅调用 clazz.getMethod(methodName, paramTypes) | getMethods()
