...
public abstract class AbstractAutoProxyCreator extends ProxyProcessorSupport
	...
	/** CacheKey -> 代理对象的实际Class */
	private final Map<Object, Class<?>> proxyTypes = new ConcurrentHashMap<>(16);

    /**
     * Bean 是否需要进行切面的集合
     *
     * key：经过 {@link #getCacheKey(Class, String)} 处理的 beanName
     * value：是否需要进行切面
     */
	...
	public Object postProcessBeforeInstantiation(Class<?> beanClass, String beanName) {
		...
		// 实例化前处理：
		// 1.对不需要代理的对象进行一些处理
		// 2.存在自定义的目标源，则获取目标对象进行代理，返回该代理对象
		...
		// 如果有自定义的目标源，则会在 实例化前处理 的时候就返回一个代理对象
	...
	/**
     * 判断指定类，是否代表 infrastructure 类，无需被代理
     *
	...
	protected Object createProxy(Class<?> beanClass, @Nullable String beanName,
		...
		// 将 ProxyConfig 类的属性，拷贝到 ProxyFactory 中
		...
		if (!proxyFactory.isProxyTargetClass()) {
		    // 全局的定义默认使用JDK，但是当前beanName指定使用CGLIB的情况
	...
	/**
	 * 是否需要再进行类过滤，切入点表达式包含了，所以子类默认为true
	 *
